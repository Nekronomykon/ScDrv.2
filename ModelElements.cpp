...
const vtkIdType ModelElements::NumTable = sizeof(ModelElements::TableForm) / sizeof(short);
...
QVariant ModelElements::data(const QModelIndex & index, int role) const
{
  ...
  if (ofs < NumTable && role == Qt::DisplayRole)
  {
    vtkIdType idElem = TableForm[ofs];
    if (ofs >= 0)
    {
      res = QVariant(tr(elements_->GetSymbol(idElem)));
    }
  }
    return res;
}
